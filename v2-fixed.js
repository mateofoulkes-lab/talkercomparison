...
function setDiag(){const mapped=Object.keys(targetBones).length;let animated=0;const f=state.motion?.frames?.[0];for(const j of Object.keys(JOINT_PARENT))if(f?.joints?.[j]?.rotation)animated++;$('#diag').textContent=`Rig Xbot: ${mapped}/19 huesos · JSON: ${animated}/19 joints animados · rotación-only retarget`;}

const sourceRoot=new THREE.Group();sourceRoot.visible=false;scene.add(sourceRoot);
const sourceBones={};
for(const [joint,parentName] of Object.entries(JOINT_PARENT)){const b=new THREE.Bone();b.name=joint;b.position.fromArray(REST[joint]);sourceBones[joint]=b;if(parentName)sourceBones[parentName].add(b);else sourceRoot.add(b);}
const sourceSkeleton=new THREE.Skeleton(Object.values(sourceBones));sourceRoot.updateMatrixWorld(true);

let modelRoot=null,targetSkin=null;
const targetBones={};
let retargetNames={};
let targetAllBones=[];
const restBonePos=new Map(),restBoneScale=new Map();
let restModelPos=new THREE.Vector3(),restModelScale=new THREE.Vector3(1,1,1);

new GLTFLoader().load(MODEL_URL,gltf=>{
  modelRoot=gltf.scene;scene.add(modelRoot);
  modelRoot.traverse(o=>{if(o.isMesh){o.frustumCulled=false;o.castShadow=false;o.receiveShadow=false;}if(o.isSkinnedMesh&&!targetSkin)targetSkin=o;if(o.isBone)targetAllBones.push(o);});
  for(const joint of Object.keys(JOINT_PARENT)){const b=findBone(joint,targetAllBones);if(b)targetBones[joint]=b;}
  ...
  restModelPos.copy(modelRoot.position);restModelScale.copy(modelRoot.scale);
  for(const b of targetAllBones){restBonePos.set(b,b.position.clone());restBoneScale.set(b,b.scale.clone());}
  ...
  if(missing.length)console.warn('Bones faltantes:',missing,targetAllBones.map(b=>b.name));setDiag();if(state.motion)applyMotion(currentTime());
...
function filteredLocal(j,A,B,alpha,t){let q=new THREE.Quaternion();if(A?.rotation){q.fromArray(A.rotation);if(B?.rotation)q.slerp(new THREE.Quaternion().fromArray(B.rotation),alpha);q.normalize();}const g=Math.max(0,Math.min(1.5,jointGain(j)));q.slerpQuaternions(new THREE.Quaternion(),q,g).normalize();const smooth=state.gains.smooth;if(smooth<=.001)return q;const prev=state.filtered.get(j),jumped=state.lastTime!=null&&(t<state.lastTime-.05||Math.abs(t-state.lastTime)>.25);if(!prev||jumped){state.filtered.set(j,q.clone());return q;}const follow=Math.max(.04,1-smooth*.94);prev.slerp(q,follow).normalize();return prev.clone();}
function driveSource(s,t){for(const j of Object.keys(JOINT_PARENT)){const A=s.a?.joints?.[j],B=s.b?.joints?.[j]||A;sourceBones[j].quaternion.copy(filteredLocal(j,A,B,s.alpha,t));}state.lastTime=t;sourceRoot.updateMatrixWorld(true);}

function restoreSizeTransforms(){
  modelRoot.position.copy(restModelPos);modelRoot.scale.copy(restModelScale);
  for(const b of targetAllBones){const p=restBonePos.get(b),s=restBoneScale.get(b);if(p)b.position.copy(p);if(s)b.scale.copy(s);}
}
function applyMotion(t){
  if(!targetSkin||!state.motion)return;
  const s=sample(t);if(!s)return;
  driveSource(s,t);
  restoreSizeTransforms();
  SkeletonUtils.retarget(targetSkin,sourceSkeleton,{names:retargetNames,hip:'hips',hipInfluence:new THREE.Vector3(0,0,0),preserveBonePositions:true,preserveBoneMatrix:false});
  // SkeletonUtils can import source-space translations/scales. We only want rotation.
  restoreSizeTransforms();
  modelRoot.updateMatrixWorld(true);
