...
const JOINT_PARENT={
  ...
  rightUpperLeg:'hips',rightLowerLeg:'rightLowerLeg',rightFoot:'rightLowerLeg'
};
...
const state={
  motion:null,duration:0,playing:false,playhead:0,startedAt:0,
  audioUrl:null,hasAudio:false,
  gains:{global:1,torso:1,arms:1,head:1,legs:1,smooth:.15},
  filtered:new Map(),lastTime:null,
...
function currentTime(){
  if(state.hasAudio){const a=$('#audio');return Math.min(state.duration,Number(a.currentTime)||0);}
  if(!state.playing)return state.playhead;
  return Math.min(state.duration,state.playhead+(performance.now()-state.startedAt)/1000);
}
function seek(t){
  t=Math.max(0,Math.min(state.duration,Number(t)||0));
  state.playhead=t;state.startedAt=performance.now();
  if(state.hasAudio){const a=$('#audio');if(Number.isFinite(a.duration)&&a.duration>0)a.currentTime=Math.min(t,a.duration);}
  resetFilter();
}
async function setPlaying(on){
  ...
  if(on){
    if(now>=state.duration-.001)seek(0); else {state.playhead=now;state.startedAt=performance.now();}
    state.playing=true;
    if(state.hasAudio){try{await $('#audio').play();}catch(e){console.warn(e);state.playing=false;}}
  }else{
    state.playhead=now;state.startedAt=performance.now();state.playing=false;
...
function sample(t){
  ...
  return {a:frames[a],b:frames[b],alpha:f-a,index:a};
}
...
function filteredQuat(j,A,B,alpha,t){
  ...
  const g=Math.max(0,Math.min(1.5,jointGain(j)));
  q.slerpQuaternions(new THREE.Quaternion(),q,g).normalize();
  const smooth=state.gains.smooth;
  if(smooth<=.001)return q;
  const prev=state.filtered.get(j);
  const jumped=state.lastTime!=null&&(t<state.lastTime-.05||Math.abs(t-state.lastTime)>.25);
  if(!prev||jumped){state.filtered.set(j,q.clone());return q;}
  const follow=Math.max(.04,1-smooth*.94);
  prev.slerp(q,follow).normalize();
  return prev.clone();
}
...
function applyMotion(t){
  ...
  for(const [j,b] of Object.entries(bones))b.quaternion.copy(restLocal[j]);
  for(const j of Object.keys(JOINT_PARENT)){
    const b=bones[j];if(!b)continue;
    const A=s.a?.joints?.[j],B=s.b?.joints?.[j]||A;
    if(!A?.rotation)continue;
    const delta=filteredQuat(j,A,B,s.alpha,t);
    // EMAGE JSON stores LOCAL joint deltas. Apply them on top of Xbot bind pose.
    b.quaternion.copy(restLocal[j]).multiply(delta).normalize();
  }
  state.lastTime=t;
  modelRoot.updateMatrixWorld(true);
...
function captureFeet(){
  if(!modelRoot||!state.motion)return;
  modelRoot.updateMatrixWorld(true);
  state.footTargets.left=footWorld('left');state.footTargets.right=footWorld('right');
  $('#recaptureFeet').disabled=!(state.footTargets.left&&state.footTargets.right);
}
function rotateBoneToward(bone,effector,target){
  const pivot=bone.getWorldPosition(new THREE.Vector3());
  const from=effector.getWorldPosition(new THREE.Vector3()).sub(pivot);
  ...
  const desired=deltaWorld.multiply(world);
...
function solveLeg(side,target){
  ...
  for(let i=0;i<6;i++){
    rotateBoneToward(lower,foot,target);modelRoot.updateMatrixWorld(true);
    rotateBoneToward(upper,foot,target);modelRoot.updateMatrixWorld(true);
...
$('#motionFile').addEventListener('change',async e=>{
  ...
  try{
    ...
    state.motion=data;state.duration=data.frames.length/(Number(data.fps)||30);state.playing=false;seek(0);
    ...
    applyMotion(0);if(state.footIK)captureFeet();
...
$('#audioFile').addEventListener('change',e=>{
  ...
  state.audioUrl=URL.createObjectURL(file);const a=$('#audio');a.src=state.audioUrl;state.hasAudio=true;
  $('#audioName').textContent=file.name;$('#clearAudio').disabled=false;
  a.addEventListener('loadedmetadata',()=>{if(Number.isFinite(a.duration)&&a.duration>0)a.currentTime=Math.min(state.playhead,a.duration);},{once:true});
});
$('#clearAudio').addEventListener('click',()=>{
  const t=currentTime(),a=$('#audio');a.pause();a.removeAttribute('src');a.load();
  if(state.audioUrl)URL.revokeObjectURL(state.audioUrl);state.audioUrl=null;state.hasAudio=false;state.playhead=t;state.startedAt=performance.now();
  $('#audioFile').value='';$('#audioName').textContent='Sin audio · usa reloj interno';$('#clearAudio').disabled=true;
});
$('#audio').addEventListener('ended',()=>setPlaying(false));
$('#play').addEventListener('click',()=>setPlaying(!state.playing));
$('#restart').addEventListener('click',()=>{state.playing=false;$('#audio').pause();seek(0);applyMotion(0);if(state.footIK)captureFeet();});
$('#timeline').addEventListener('input',e=>{seek(Number(e.target.value)*state.duration);applyMotion(currentTime());});
...
for(const [id,key,out,mult] of sliders){
  $('#'+id).addEventListener('input',e=>{state.gains[key]=Number(e.target.value);$('#'+out).textContent=mult?`${state.gains[key].toFixed(2)}×`:state.gains[key].toFixed(2);resetFilter();applyMotion(currentTime());});
}
$('#resetGains').addEventListener('click',()=>{
  const d={global:1,torso:1,arms:1,head:1,legs:1,smooth:.15};
  for(const [id,key,out,mult] of sliders){state.gains[key]=d[key];$('#'+id).value=String(d[key]);$('#'+out).textContent=mult?`${d[key].toFixed(2)}×`:d[key].toFixed(2);}resetFilter();applyMotion(currentTime());
});
$('#footIK').addEventListener('change',e=>{
  ...
  if(state.footIK){applyMotion(currentTime());captureFeet();}else{$('#recaptureFeet').disabled=true;applyMotion(currentTime());}
});
$('#recaptureFeet').addEventListener('click',()=>{applyMotion(currentTime());captureFeet();});

for(const btn of document.querySelectorAll('.view'))btn.addEventListener('click',()=>{
  document.querySelectorAll('.view').forEach(b=>b.classList.toggle('active',b===btn));
  const v=btn.dataset.view;
  if(v==='front')camera.position.set(0,1.18,4);
  if(v==='side')camera.position.set(4,1.18,0);
  if(v==='threequarter')camera.position.set(2.7,1.35,2.7);
  orbit.target.set(0,1.05,0);orbit.update();
});

function animate(){
  requestAnimationFrame(animate);
  const t=currentTime();
  if(state.playing&&!state.hasAudio&&t>=state.duration){state.playing=false;state.playhead=state.duration;}
  if(state.motion)applyMotion(t);
  orbit.update();syncUi();renderer.render(scene,camera);
}
animate();
